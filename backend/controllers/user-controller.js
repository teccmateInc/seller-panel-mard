...
exports.UpdatePassword = async (req, res) => {
  try {
    ...
    if (password.length < 8) {
      return handleErrorWithStatus(res, 200, 'Password must be 8 digits!')
    }
    updatedPassword = {
    ...
    const passwordMatched = await user.comparePassword(password)
    if (!passwordMatched) {
      user = await User.findByIdAndUpdate(req.user._id, updatedPassword, {
        new: true,
        runValidators: true,
      ...
      handleErrorWithStatus(
        res,
        200,
        'Password is matched from your current password!',
        {},
        'info'
      )
  } catch (err) {
